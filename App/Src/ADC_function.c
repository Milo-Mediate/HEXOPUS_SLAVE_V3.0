...
static const uint16_t adc_dma_len[] = { NUM_SENS, NUM_DSP };

static const uint32_t volatile * adcs[] = {adc1_data, adc2_data};

#define ASSERT_PORT(port) assert((port) < sizeof(adcs)/sizeof(adcs[0]) && "port must be 0 or 1")
